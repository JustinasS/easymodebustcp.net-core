...
namespace EasyModbusAdvancedClient
{
	...
	public partial class MainForm : Form
	{
		...
		void StartSingleJobToolStripMenuItemClick(object sender, EventArgs e)
		{
            ...
            getValuesThreadObject.connectionProperty.modbusClient.ReceiveDataChanged -= new ModbusClient.ReceiveDataChangedHandler(UpdateTextBoxReceive);
            getValuesThreadObject.connectionProperty.modbusClient.SendDataChanged -= new ModbusClient.SendDataChangedHandler(UpdateTextBoxSend);
		...
		{		
			foreach(ConnectionProperties connectionProperty in easyModbusManager.connectionPropertiesList)
			{
                connectionProperty.modbusClient.ReceiveDataChanged += new ModbusClient.ReceiveDataChangedHandler(UpdateTextBoxReceive);
                connectionProperty.modbusClient.SendDataChanged += new ModbusClient.SendDataChangedHandler(UpdateTextBoxSend);
                GetValuesThreadObject getValuesThreadObject = new GetValuesThreadObject();
    ...
    private void stopCurrentJobToolStripMenuItem_Click(object sender, EventArgs e)
    {
        ...
        if (treeView1.SelectedNode.Level == 1)
        {
            ...
            easyModbusManager.connectionPropertiesList[indexJobToStop].modbusClient.Disconnect();
                easyModbusManager.connectionPropertiesList[indexJobToStop].modbusClient.ReceiveDataChanged += new ModbusClient.ReceiveDataChangedHandler(UpdateTextBoxReceive);
                easyModbusManager.connectionPropertiesList[indexJobToStop].modbusClient.SendDataChanged += new ModbusClient.SendDataChangedHandler(UpdateTextBoxSend);
            }
        if (treeView1.SelectedNode.Level == 2)
            {
        ...
        delegate void UpdateReceiveDataCallback(ModbusClient sender);
        ...
        delegate void UpdateSendDataCallback(ModbusClient sender);
